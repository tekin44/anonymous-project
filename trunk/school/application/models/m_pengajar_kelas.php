...
class m_pengajar_kelas extends CI_Model {
	...
	public function show_pengajar_kelas($id) {
		$this->load->database();
		$query = $this->db->query("select * from pengajar_kelas a " .
				"INNER JOIN kelas b on a.id_kelas = b.id_kelas " .
				"INNER JOIN pengajar c on a.nomor_induk_pengajar = c.nomor_induk_pengajar " .
				"INNER JOIN pelajaran d on c.kode_pelajaran = d.kode_pelajaran " .
				"WHERE a.id_kelas = '".$id."'");
		return $query->result();
	}
